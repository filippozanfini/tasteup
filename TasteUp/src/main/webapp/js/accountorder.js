         ...
         $.each(ordini.ordini, function (key, value) {
                   ...
                   <td><h5 class="card-text" id="radiobtn" value=' +
                     ...
                     <div id="dati">\
                       ...
                       '<div id ="blocco">\
                         ...
                         <h5 id="nominativi_indirizzi">'+ value.totale +"&euro;</h5></td>\
                       </div>\
                    </div>"+
   
                   '<div id="buttons">'+
                   '<button id="aggiungi" data-toggle="modal" onclick="getOrdine('+value.id+')" data-target="#addressModal'+ value.id+'"><span>Dettaglio</span></button>\
                   <button  id="chiudi" onclick="rimuovi_consegna(\'' +
                   value.id +'\')" ><span>Annulla</span></button>\
                   </div>\
                 </div>\
                 </div>'
                 );
          ...
          $.each(ordini.ordini, function (key, value) {
            ...
            if (value.fase == "true") {
             ...
             $.each(ordini.ordini, function (key, value) {
              $("#mod").append(
             '<div id="addressModal'+value.id+'" class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">\
             <div class="modal-dialog">\
               <div class="modal-content">\
                 <div class="modal-header">\
                   <h5 class="modal-title" id="exampleModalLabel"> Ordine: #'+ value.id +'</h5>'+
                   '<button type="button" class="close" data-dismiss="modal" aria-label="Close">\
                     <span aria-hidden="true">&times;</span>\
                   </button>\
                 </div>\
                 <div id="addressbody'+value.id+'"class="modal-body">\
                 </div>\
                 <div id="tot'+value.id+'"class="tot">\
                 </div>\
...
function getOrdine(id){
  ...
  $.each(ordine.ordine, function (key, value) {
    document.getElementById('tot'+id).innerHTML="<h5 id='tot'> Totale:"+ value.totale +"&euro; </h5>" ;
    $.each(value.panini, function (key, val) {
      if(value.id == id) 
        $(m).append('<div id="div_popup">\
                      <div id="div_img">\
                      <img id="img_popup" src="../../assets/panini/' +
                        val.immagine + '.png" class="product-image">\
                        </div>'+

                   
                    '<div id="div_prodotto">\
                     <h5 id="nominativi">'+val.nome_panino+'</h5>'+
                    '<h5 id="nom">Quantit&agrave: '+val.quantita +'</h5>'+
                    '<h5 id="nom">Prezzo: '+val.prezzo +'&euro;</h5>'+
                    '</div>'+ 
                    '</div>'
                    );
    ...
    $.each(value.menu, function (key, val) {
      if(value.id == id){
        ...
        <img id="img_popup" src="../../assets/menu/' +
          ...
          '<div id="div_prodotto">\
          <h5 id="nominativi">'+val.nome_menu+'</h5>'+
          '<h5 id="nom">Quantit&agrave: '+val.quantita +'</h5>'+
          '<h5 id="nom">Prezzo: '+val.prezzo +'&euro;</h5>'+
          '</div></div>'
          );
  ...
  $.each(ordine.ordine, function (key, value) {
    $.each(value.bevande, function (key, val) {
      if(value.id == id) { 
        $(m).append('<div id="div_popup">'+'<div id="div_img">\
      <img id="img_popup" src="../../assets/bevande/' +
        val.immagine + '.png" class="product-image">\
         </div>'+
        '<div id="div_prodotto">\
         <h5 id="nominativi">'+val.nome_bevanda+'</h5>'+
        '<h5 id="nom">Quantit&agrave: '+val.quantita +'</h5>'+
        '<h5 id="nom">Prezzo: '+val.prezzo +'&euro;</h5>'+
        '</div></div>'
        );
